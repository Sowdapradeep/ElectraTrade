...
const ShopCatalog = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 flex gap-12 relative">
      {/* Filter Sidebar */}
      ...
      <div className="flex-1 space-y-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map(p => (
            <div key={p.id} className="bg-white rounded-[32px] border border-slate-100 overflow-hidden flex flex-col group hover:shadow-2xl transition-all duration-500">
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{p.brand}</span>
                  <button onClick={() => toggleCompare(p)} className={`text-[9px] font-black uppercase tracking-widest hover:text-blue-600 ${comparing.find(item => item.id === p.id) ? 'text-blue-600' : 'text-slate-400'}`}>
                     {comparing.find(item => item.id === p.id) ? 'Selected' : '+ Compare'}
                  </button>
                </div>
                <h3 className="font-bold text-slate-900 mb-6 text-lg">{p.name}</h3>
                
                <div className="mt-auto space-y-6">
                  <div className="flex justify-between items-end">
                     <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Wholesale Unit</p>
                        <p className="text-3xl font-black text-slate-900">${p.price.toLocaleString()}</p>
                     </div>
                     <span className="text-[10px] font-bold text-slate-400">MOQ: {p.moq}u</span>
                  </div>
                  <button 
                    onClick={() => addToCart({ productId: p.id, product: p, quantity: p.moq })}
                    className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 transition-all active:scale-95"
                  >
